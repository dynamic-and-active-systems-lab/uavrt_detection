...
void sum(const sparse *x, b_sparse *y)
{
  static rtDoubleCheckInfo
      t_emlrtDCI{
          ...
          "sum", // fName
          "C:\\Program "
          "Files\\MATLAB\\toolbox\\eml\\lib\\matlab\\datafun\\sum.m", // pName
          4 // checkKind
      };
  if ((x->m == 0) || (x->n == 0) || (x->m == 0)) {
    ...
    for (int col{0}; col < b; col++) {
      ...
      r = 0.0;
      if ((u1 <= x->colidx[col + 1] - 1) &&
          (x->colidx[col + 1] - 1 > 2147483646)) {
        check_forloop_overflow_error();
      }
      for (int xp{u1}; xp <= xend; xp++) {
        r += x->d[xp - 1];
      }
      if (r != 0.0) {
        y->d[outidx - 1] = r;
    ...
    b = y->colidx[y->colidx.size(0) - 1];
    if (y->colidx[y->colidx.size(0) - 1] - 1 > 2147483646) {
      check_forloop_overflow_error();
    }
    for (outidx = 0; outidx <= b - 2; outidx++) {
      y->rowidx[outidx] = 1;
    }
  }
}
