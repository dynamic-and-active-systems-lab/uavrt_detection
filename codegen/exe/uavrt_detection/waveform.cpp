  ...
  boolean_T, 2U> &indiv_msk, coder::array<double, 1U> &peak_ind) const
{
  ...
  for (wind_start_size = 0; wind_start_size < i; wind_start_size++) {
    ...
    timeSearchRange[(wind_start_size + timeSearchRange.size(0)) + 1] =
      ...
      (wind_start_size) + 2.0) - 1.0) * naive_wind_end_tmp + b_J);
  }
  ...
  if ((r2.size(0) == msk.size(0)) && (r2.size(1) == msk.size(1))) {
    ...
    i_binary_expand_op(excld_msk_vec, r2, msk);
  }
  ...
  if (loop_ub == timeBlinderVec.size(0)) {
    ...
    g_binary_expand_op(greater_than_next, scores, loop_ub - 1, timeBlinderVec);
  }
  ...
  if (loop_ub == timeBlinderVec.size(0)) {
    ...
    g_binary_expand_op(excld_msk_vec, scores, loop_ub - 1, timeBlinderVec);
  }
  ...
  if (b_freq_mask.size(0) == r9.size(0)) {
    ...
    f_binary_expand_op(excld_msk_vec, b_freq_mask, r9);
  }
  ...
  if (b_freq_mask.size(0) == r9.size(0)) {
    ...
    e_binary_expand_op(score_right_bndry, b_freq_mask, r9);
  }
  ...
  do {
    ...
    if (y) {
      ...
      for (int k{0}; k < i1; k++) {
        ...
        loop_ub = timeBlinderVec.size(0);
        for (i2 = 0; i2 < loop_ub; i2++) {
          b_freq_mask[i2] = (timeBlinderVec[i2] <= t_lastknown);
        }

        r9.set_size(indsOfBinsValid.size(0));
        loop_ub = indsOfBinsValid.size(0);
        for (i2 = 0; i2 < loop_ub; i2++) {
          r9[i2] = (indsOfBinsValid[i2] <= t_lastknown);
        }
        ...
        for (i2 = 0; i2 < loop_ub; i2++) {
          r9[i2] = (n_diff_check_for[i2] <= t_lastknown);
        }
  ...
  makepulsestruc(&r10);
  coder::repmat(&r10, static_cast<double>(yw_max_all_freq.size(0)), static_cast<
  ...
  loop_ub = r11.size(0);
  for (i = 0; i < loop_ub; i++) {
    if (r11[i] > maxStftTimeWind) {
      rtDynamicBoundsError(r11[i], 1, maxStftTimeWind, &we_emlrtBCI);
    }
  }
  ...
  r13.set_size(r12.size(0), yw_max_all_freq.size(1));
  for (i = 0; i < loop_ub; i++) {
    maxStftTimeWind = r12.size(0);
    for (i1 = 0; i1 < maxStftTimeWind; i1++) {
      i2 = static_cast<int>(b_excluded_freq_bands[(r12[i1] + num[0] * i) - 1]);
      if (i2 > nx) {
        rtDynamicBoundsError(i2, 1, nx, &af_emlrtBCI);
      }

      r13[i1 + r13.size(0) * i] = i2;
    }
  }
  ...
  c_S_cols.set_size(r14.size(0), S_cols.size(1));
  for (i = 0; i < loop_ub; i++) {
    maxStftTimeWind = r14.size(0);
    for (i1 = 0; i1 < maxStftTimeWind; i1++) {
      c_S_cols[i1 + c_S_cols.size(0) * i] = S_cols[(r14[i1] + S_cols.size(0) * i)
        - 1];
    }
  }

  nx = r14.size(0) * S_cols.size(1);
  ...
  for (i = 0; i < loop_ub; i++) {
    refmat[r13[i] - 1] = (timeBlinderVec[i] - tp_temp) + t_0;
  }
  ...
  for (wind_start_size = 0; wind_start_size < i; wind_start_size++) {
    for (nx = 0; nx < n_freqs; nx++) {
      ...
      makepulsestruc(searchmat[nx + searchmat.size(0) * wind_start_size],
                     ...
                     refmat.size(0) * wind_start_size], refmat[nx + refmat.size
                     (0) * wind_start_size] + ps_pre->t_p, t_srch_rng, Wf[nx],
  ...
  &excluded_freq_bands)
{
  ...
  static rtDoubleCheckInfo t_emlrtDCI{ 1842,// lineNo
    51,                                // colNo
    "waveform/process",                // fName
    "H:\\repos\\uavrt_detection\\waveform.m",// pName
    1                                  // checkKind
  ...
  static rtDoubleCheckInfo v_emlrtDCI{ 1686,// lineNo
    51,                                // colNo
    "waveform/process",                // fName
    "H:\\repos\\uavrt_detection\\waveform.m",// pName
    1                                  // checkKind
   ...
   case 0:
    {
      ...
      if (coder::internal::ifWhileCond(r)) {
        ...
        if (pk_ind[0] != static_cast<int>(std::floor(pk_ind[0]))) {
          rtIntegerError(pk_ind[0], &v_emlrtDCI);
        }
        ...
        coder::internal::validator_check_size(c_candidatelist, val);
      ...
      if (coder::internal::ifWhileCond(r)) {
        ...
        for (int b_i{0}; b_i <= n; b_i++) {
          val.set_size(1, ps_pos->pl.size(1));
          ...
          if (b_i + 1 > val.size(1)) {
            rtDynamicBoundsError(b_i + 1, 1, val.size(1), &xb_emlrtBCI);
          }

          val[b_i].con_dec = false;
          obj = ps_pos;
          c_candidatelist.set_size(1, val.size(1));
          b_n = val.size(0) * val.size(1) - 1;
          for (i = 0; i <= b_n; i++) {
            c_candidatelist[i] = val[i];
          }
      ...
      for (int b_i{0}; b_i <= n; b_i++) {
        ...
        if (b_i + 1 > val.size(1)) {
          rtDynamicBoundsError(b_i + 1, 1, val.size(1), &yb_emlrtBCI);
        }

        val[val.size(0) * b_i].mode.set_size(1, val[val.size(0) * b_i].mode.size
          (1));
        if (b_i + 1 > val.size(1)) {
          rtDynamicBoundsError(b_i + 1, 1, val.size(1), &yb_emlrtBCI);
        }

        val[val.size(0) * b_i].mode.set_size(val[val.size(0) * b_i].mode.size(0),
          1);
        if (b_i + 1 > val.size(1)) {
          rtDynamicBoundsError(b_i + 1, 1, val.size(1), &yb_emlrtBCI);
        }
   ...
   case 1:
    {
      ...
      if (have_priori_freq_band) {
        ...
        for (int b_i{0}; b_i <= n; b_i++) {
          ...
          if (b_i + 1 > val.size(1)) {
            rtDynamicBoundsError(b_i + 1, 1, val.size(1), &rb_emlrtBCI);
          }

          val[b_i].con_dec = conflog[b_i];
          obj = ps_pos;
          c_candidatelist.set_size(1, val.size(1));
          b_n = val.size(0) * val.size(1) - 1;
          for (i = 0; i <= b_n; i++) {
      ...
      for (int b_i{0}; b_i <= n; b_i++) {
        ...
        if (b_i + 1 > val.size(1)) {
          rtDynamicBoundsError(b_i + 1, 1, val.size(1), &sb_emlrtBCI);
        }
   ...
   case 2:
    {
      ...
      if (coder::internal::ifWhileCond(r)) {
        ...
        if (have_priori_freq_band) {
          ...
          for (int b_i{0}; b_i <= n; b_i++) {
            ...
            if (b_i + 1 > val.size(1)) {
              rtDynamicBoundsError(b_i + 1, 1, val.size(1), &kb_emlrtBCI);
            }
      ...
      for (int b_i{0}; b_i <= n; b_i++) {
        ...
        if (b_i + 1 > val.size(1)) {
          rtDynamicBoundsError(b_i + 1, 1, val.size(1), &lb_emlrtBCI);
        }
      ...
      for (int b_i{0}; b_i <= n; b_i++) {
        ...
        if ((static_cast<int>(b_i + 1U) < 1) || (static_cast<int>(b_i + 1U) > i))
        {
          rtDynamicBoundsError(static_cast<int>(b_i + 1U), 1, i, &nb_emlrtBCI);
        }
...
void waveform::setweightingmatrix()
{
  ...
  static rtDoubleCheckInfo t_emlrtDCI{ 43,// lineNo
    28,                                // colNo
    "gettemplate",                     // fName
  ...
  static rtDoubleCheckInfo v_emlrtDCI{ 155,// lineNo
    1,                                 // colNo
    "weightingmatrix",                 // fName
    "H:\\repos\\uavrt_detection\\weightingmatrix.m",// pName
    1                                  // checkKind
  ...
  if (loop_ub < 0) {
    rtNonNegativeError(static_cast<double>(loop_ub), &t_emlrtDCI);
  }
  ...
  xtmp_im = 2.0 * static_cast<double>(x_of_n.size(0));
  if (xtmp_im != static_cast<int>(xtmp_im)) {
    rtIntegerError(xtmp_im, &v_emlrtDCI);
  }
  ...
  for (int b_i{0}; b_i < 2; b_i++) {
    ...
    if (Xs.size(0) < 1) {
      rtDynamicBoundsError(1, 1, Xs.size(0), &ib_emlrtBCI);
    }
