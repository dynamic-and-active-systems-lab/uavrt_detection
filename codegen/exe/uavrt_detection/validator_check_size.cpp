...
//
// Arguments    : const bool in_data[]
//                int in_size
// Return Type  : bool
//
namespace coder {
namespace internal {
bool b_validator_check_size(const bool in_data[], int in_size)
{
  char tmp_data[10];
  bool out;
  if (in_size != 1) {
    int tmp_size[2];
    int2str(static_cast<double>(0.0), tmp_data, tmp_size);
    rtErrorWithMessageID(std::string(&tmp_data[0], tmp_size[1]), "1", "1",
                         k_emlrtRTEI.fName, k_emlrtRTEI.lineNo);
  } else {
    out = in_data[0];
  }
  return out;
...
//
// Arguments    : const ::coder::array<bool, 2U> &in
//                ::coder::array<bool, 2U> &out
// Return Type  : void
//
void d_validator_check_size(const ::coder::array<bool, 2U> &in,
                            ::coder::array<bool, 2U> &out)
{
  int sizes_idx_0;
  int sizes_idx_1;
