                ...
                coder::array<creal_T, 2U> &outMat)
{
  static rtBoundsCheckInfo ac_emlrtBCI{
      ...
      "str2matrix",                                                 // fName
      "H:\\repos\\uavrt_detection\\detector\\config\\str2matrix.m", // pName
      0                                                             // checkKind
  };
  ...
  for (int c_i{0}; c_i < nd2; c_i++) {
    if (inputStr[c_i] == ' ') {
      if (c_i + 1 > inputStr.size(1)) {
        rtDynamicBoundsError(c_i + 1, 1, inputStr.size(1), &sb_emlrtBCI);
      }
      inputStr[c_i] = ',';
    }
  }
  ...
  if (rowDividerLocs.size(1) < 1) {
    rtDynamicBoundsError(1, 1, rowDividerLocs.size(1), &tb_emlrtBCI);
  }
  if (rowDividerLocs.size(1) < 2) {
    rtDynamicBoundsError(2, 1, rowDividerLocs.size(1), &ub_emlrtBCI);
  }
  ...
  for (int c_i{0}; c_i < nd2; c_i++) {
    if (b_inputStr[c_i] && r[c_i] && (c_i + 1 > leftBracketLoc.size(1))) {
      rtDynamicBoundsError(c_i + 1, 1, leftBracketLoc.size(1), &vb_emlrtBCI);
    }
  }
  nd2 = b_inputStr.size(1);
  trueCount = 0;
  ...
  for (int c_i{0}; c_i <= b_i - 2; c_i++) {
    ...
    if (c_i + 1 > rowDividerLocs.size(1)) {
      rtDynamicBoundsError(c_i + 1, 1, rowDividerLocs.size(1), &wb_emlrtBCI);
    }
    if (c_i + 2 > rowDividerLocs.size(1)) {
      rtDynamicBoundsError(c_i + 2, 1, rowDividerLocs.size(1), &xb_emlrtBCI);
    }
    b_inputStr.set_size(1, leftBracketLoc.size(1));
    ...
    for (j2 = 0; j2 < nd2; j2++) {
      if (b_inputStr[j2] && r[j2] && (j2 + 1 > leftBracketLoc.size(1))) {
        rtDynamicBoundsError(j2 + 1, 1, leftBracketLoc.size(1), &yb_emlrtBCI);
      }
    }
    nd2 = b_inputStr.size(1);
    trueCount = 0;
    ...
    for (j2 = 0; j2 <= i1 - 2; j2++) {
      if (j2 + 1 > rightBracketLoc.size(1)) {
        rtDynamicBoundsError(j2 + 1, 1, rightBracketLoc.size(1), &ac_emlrtBCI);
      }
      if (j2 + 2 > rightBracketLoc.size(1)) {
        rtDynamicBoundsError(j2 + 2, 1, rightBracketLoc.size(1), &bc_emlrtBCI);
      }
      ...
      if (rightBracketLoc[j2] + 1.0 > b_rowDividerLocs) {
        ...
        if ((nd2 < 1) || (nd2 > inputStr.size(1))) {
          rtDynamicBoundsError(nd2, 1, inputStr.size(1), &dc_emlrtBCI);
        }
        nd2--;
      ...
      if (j2 + 1 > tempRow.size(1)) {
        rtDynamicBoundsError(j2 + 1, 1, tempRow.size(1), &cc_emlrtBCI);
      }
      tempRow[j2] = coder::str2double(c_inputStr);
    }
    if (c_i + 1 > outMat.size(0)) {
      rtDynamicBoundsError(c_i + 1, 1, outMat.size(0), &fc_emlrtBCI);
    }
    iv2[1] = outMat.size(1);
