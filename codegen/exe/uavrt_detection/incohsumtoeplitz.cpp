                      ...
                      coder::array<double, 2U> &Scols)
{
  ...
  if (S.size(0) != Wfherm.size(1)) {
    if (((Wfherm.size(0) == 1) && (Wfherm.size(1) == 1)) ||
      ...
      vb_rtErrorWithMessageID(cc_emlrtRTEI.fName, cc_emlrtRTEI.lineNo);
    } else {
      mb_rtErrorWithMessageID(bc_emlrtRTEI.fName, bc_emlrtRTEI.lineNo);
    }
  }
  ...
  for (int idx{0}; idx < Fbnumrows_tmp; idx++) {
    ...
    if ((b_minval_tmp < 1) || (b_minval_tmp > cend)) {
      rtDynamicBoundsError(b_minval_tmp, 1, cend, &ib_emlrtBCI);
    }
  ...
  for (int idx{0}; idx < Fbnumrows_tmp; idx++) {
    ...
    for (b_minval_tmp = 0; b_minval_tmp < cend; b_minval_tmp++) {
      if (r4[b_minval_tmp] > Sinds.size(0)) {
        rtDynamicBoundsError(r4[b_minval_tmp], 1, Sinds.size(0), &hb_emlrtBCI);
      }
  ...
  for (int idx{0}; idx < Fbnumrows_tmp; idx++) {
    ...
    for (b_minval_tmp = 0; b_minval_tmp < cend; b_minval_tmp++) {
      if (r5[b_minval_tmp] > maxdimlen) {
        rtDynamicBoundsError(r5[b_minval_tmp], 1, maxdimlen, &gb_emlrtBCI);
      }
