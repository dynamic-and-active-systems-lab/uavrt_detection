               ...
               const c_anonymous_function *cmp)
{
  ...
  if (xend > 1) {
    if (xend <= 32) {
      ...
      if (unnamed_idx_0 <= 0) {
        lc_rtErrorWithMessageID(nc_emlrtRTEI.fName, nc_emlrtRTEI.lineNo);
      }
      st.d.data[0] = frame;
      st.n = 1;
      while (st.n > 0) {
        ...
        if (pmin + 1 <= 32) {
          ...
          int pivot;
          pmax = (frame.xstart + ((pmin + (pmin < 0)) >> 1)) - 1;
    ...
    if (xend <= 32) {
      ...
      while (st.n > 0) {
        ...
        if (i + 1 <= 32) {
          ...
          bool varargout_1;
          pmin = (frame.xstart + ((i + (i < 0)) >> 1)) - 1;
          i = x[frame.xstart - 1];
          pmax = cmp->workspace.a[i - 1];
          if (cmp->workspace.a[x[pmin] - 1] < pmax) {
            varargout_1 = true;
          } else if (cmp->workspace.a[x[pmin] - 1] == pmax) {
            varargout_1 =
                (cmp->workspace.b[x[pmin] - 1] < cmp->workspace.b[i - 1]);
          } else {
            varargout_1 = false;
          }
          if (varargout_1) {
          ...
          if (cmp->workspace.a[x[frame.xend - 1] - 1] <
              cmp->workspace.a[x[pmin] - 1]) {
            varargout_1 = true;
          } else {
            i = x[frame.xend - 1] - 1;
            if (cmp->workspace.a[i] == cmp->workspace.a[x[pmin] - 1]) {
              varargout_1 =
                  (cmp->workspace.b[i] < cmp->workspace.b[x[pmin] - 1]);
            } else {
              varargout_1 = false;
            }
          }
          ...
          do {
            ...
            do {
              ...
              if (i < cmp->workspace.a[pow2p]) {
                ...
                varargout_1 =
                    (cmp->workspace.b[x[pmin] - 1] < cmp->workspace.b[pow2p]);
              } else {
                varargout_1 = false;
              }
              if (varargout_1) {
            ...
            do {
              ...
              if (cmp->workspace.a[pow2p] < i) {
                ...
                varargout_1 =
                    (cmp->workspace.b[pow2p] < cmp->workspace.b[x[j] - 1]);
              } else {
                varargout_1 = false;
              }
              if (varargout_1) {
